...
int ensure_dctx(ZstdDecompressor *decompressor, int loadDict) {
    ZstdModuleState *st = zstd_state_from_obj((PyObject *)decompressor);
    size_t zresult;
    ...
    if (decompressor->maxWindowSize) {
        ...
        if (ZSTD_isError(zresult)) {
            PyErr_Format(zstd_error(st), "unable to set max window size: %s",
    ...
    zresult = ZSTD_DCtx_setParameter(decompressor->dctx, ZSTD_d_format,
                                     decompressor->format);
    if (ZSTD_isError(zresult)) {
        PyErr_Format(zstd_error(st), "unable to set decoding format: %s",
                     ZSTD_getErrorName(zresult));
        return 1;
    }
    ...
    while (1) {
        ...
        while (input.pos < input.size) {
            ...
            Py_END_ALLOW_THREADS
                ...
                if (ZSTD_isError(zresult)) {
                PyErr_Format(zstd_error(st), "zstd decompressor error: %s",
                             ZSTD_getErrorName(zresult));
                res = NULL;
                goto finally;
            }
...
PyObject *Decompressor_decompress(ZstdDecompressor *self, PyObject *args,
    ...
    /* Size is recorded in frame header. */
        ...
        if (decompressedSize > PY_SSIZE_T_MAX) {
            PyErr_SetString(
                zstd_error(st), "frame is too large to decompress on this platform");
            goto finally;
        }
    ...
    Py_END_ALLOW_THREADS
        ...
        if (ZSTD_isError(zresult)) {
        PyErr_Format(zstd_error(st), "decompression error: %s",
                     ZSTD_getErrorName(zresult));
        Py_CLEAR(result);
        goto finally;
    }
...
static ZstdDecompressionObj *Decompressor_decompressobj(ZstdDecompressor *self,
    ...
    result = (ZstdDecompressionObj *)PyObject_CallObject(
        (PyObject *)st->ZstdDecompressionObjTypeObj, NULL);
    if (!result) {
        return NULL;
    }
...
Decompressor_read_to_iter(ZstdDecompressor *self, PyObject *args,
    ...
    result = (ZstdDecompressorIterator *)PyObject_CallObject(
        (PyObject *)st->ZstdDecompressorIteratorTypeObj, NULL);
    if (!result) {
        return NULL;
    }
...
Decompressor_stream_reader(ZstdDecompressor *self, PyObject *args,
    ...
    result = (ZstdDecompressionReader *)PyObject_CallObject(
        (PyObject *)st->ZstdDecompressionReaderTypeObj, NULL);
    if (NULL == result) {
        return NULL;
    }
...
Decompressor_stream_writer(ZstdDecompressor *self, PyObject *args,
    ...
    result = (ZstdDecompressionWriter *)PyObject_CallObject(
        (PyObject *)st->ZstdDecompressionWriterTypeObj, NULL);
    if (!result) {
        return NULL;
    }
...
Decompressor_decompress_content_dict_chain(ZstdDecompressor *self,
    ...
    Py_END_ALLOW_THREADS if (ZSTD_isError(zresult)) {
        PyErr_Format(zstd_error(st), "could not decompress chunk 0: %s",
                     ZSTD_getErrorName(zresult));
        goto finally;
    }
    else if (zresult) {
        PyErr_Format(zstd_error(st), "chunk 0 did not decompress full frame");
        goto finally;
    }
    ...
    for (chunkIndex = 1; chunkIndex < chunksLen; chunkIndex++) {
        ...
        if (parity) {
            ...
            Py_BEGIN_ALLOW_THREADS zresult =
                ZSTD_decompressStream(self->dctx, &outBuffer, &inBuffer);
            Py_END_ALLOW_THREADS if (ZSTD_isError(zresult)) {
                PyErr_Format(zstd_error(st), "could not decompress chunk %zd: %s",
...
decompress_from_framesources(ZstdDecompressor *decompressor,
    ...
    if (threadCount > 1) {
        ...
        if (NULL == pool) {
            PyErr_SetString(zstd_error(st), "could not initialize zstd thread pool");
            goto finally;
        }
    }
    ...
    Py_END_ALLOW_THREADS
        ...
        case DecompressorWorkerError_zstd:
            PyErr_Format(zstd_error(st), "error decompressing item %zd: %s",
        ...
        default:
            PyErr_Format(zstd_error(st), "unhandled error type: %d; this is a bug",
    ...
    for (i = 0; i < threadCount; i++) {
        ...
        for (bufferIndex = 0; bufferIndex < state->destCount; bufferIndex++) {
            ...
            bws = BufferWithSegments_FromMemory(st,
    ...
    result = (ZstdBufferWithSegmentsCollection *)PyObject_CallObject(
        (PyObject *)st->ZstdBufferWithSegmentsCollectionTypeObj, resultArg);
...
Decompressor_multi_decompress_to_buffer(ZstdDecompressor *self, PyObject *args,
    ...
    if (PyObject_TypeCheck(frames, st->ZstdBufferWithSegmentsTypeObj)) {
    ...
    else if (PyObject_TypeCheck(frames, st->ZstdBufferWithSegmentsCollectionTypeObj)) {
        Py_ssize_t offset = 0;
...
void decompressor_module_init(PyObject *mod) {
    ...
    st->ZstdDecompressorTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(mod, &ZstdDecompressorSpec, NULL);
    if (!st->ZstdDecompressorTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdDecompressorTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "ZstdDecompressor",
                          (PyObject *)st->ZstdDecompressorTypeObj);
}
