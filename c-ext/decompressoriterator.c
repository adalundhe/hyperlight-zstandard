...
void decompressoriterator_module_init(PyObject *mod) {
    ...
    st->ZstdDecompressorIteratorTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            mod, &ZstdDecompressorIteratorSpec, NULL);
    if (!st->ZstdDecompressorIteratorTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdDecompressorIteratorTypeObj) < 0) {
        return;
    }
}
