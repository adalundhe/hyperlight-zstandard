...
static PyObject *ZstdCompressionObj_flush(ZstdCompressionObj *self,
    ...
    if (self->finished) {
        PyErr_SetString(zstd_error(st), "compressor object already finished");
        return NULL;
    }
    ...
    default:
        PyErr_SetString(zstd_error(st), "unhandled flush mode");
        return NULL;
    }
...
void compressobj_module_init(PyObject *module) {
    ...
    st->ZstdCompressionObjTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            module, &ZstdCompressionObjSpec, NULL);
    if (!st->ZstdCompressionObjTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdCompressionObjTypeObj) < 0) {
        return;
    }
}
