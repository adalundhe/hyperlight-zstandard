...
int setup_cctx(ZstdCompressor *compressor) {
    ZstdModuleState *st = zstd_state_from_obj((PyObject *)compressor);
    size_t zresult;
    ...
    if (ZSTD_isError(zresult)) {
        PyErr_Format(zstd_error(st), "could not set compression parameters: %s",
...
static int ZstdCompressor_init(ZstdCompressor *self, PyObject *args,
    ...
    if (dict) {
        if (dict == Py_None) {
            dict = NULL;
                                      ...
                                      (PyObject *)st->ZstdCompressionDictTypeObj)) {
    ...
    if (params) {
        if (params == Py_None) {
            params = NULL;
                     ...
                     params, (PyObject *)st->ZstdCompressionParametersTypeObj)) {
            PyErr_Format(
                PyExc_TypeError,
    ...
    if (params) {
        if (set_parameters(st, self->params,
                           (ZstdCompressionParametersObject *)params)) {
            return -1;
        }
    }
    else {
        if (set_parameter(st, self->params, ZSTD_c_compressionLevel, level)) {
            return -1;
        }

        if (set_parameter(st, self->params, ZSTD_c_contentSizeFlag,
                          writeContentSize ? PyObject_IsTrue(writeContentSize)
                                           : 1)) {
            return -1;
        }

        if (set_parameter(st, self->params, ZSTD_c_checksumFlag,
                          writeChecksum ? PyObject_IsTrue(writeChecksum) : 0)) {
            return -1;
        }

        if (set_parameter(st, self->params, ZSTD_c_dictIDFlag,
                          writeDictID ? PyObject_IsTrue(writeDictID) : 1)) {
            return -1;
        }

        if (threads) {
            if (set_parameter(st, self->params, ZSTD_c_nbWorkers, threads)) {
                return -1;
            }
        }
    }
...
static PyObject *ZstdCompressor_memory_size(ZstdCompressor *self) {
    ...
    if (self->cctx) {
        ...
        PyErr_SetString(
            zstd_error(st),
            "no compressor context found; this should never happen");
        return NULL;
    }
}
...
static PyObject *ZstdCompressor_copy_stream(ZstdCompressor *self,
    ...
    if (ZSTD_isError(zresult)) {
        PyErr_Format(zstd_error(st), "error setting source size: %s",
                     ZSTD_getErrorName(zresult));
        return NULL;
    }
...
static ZstdCompressionReader *ZstdCompressor_stream_reader(ZstdCompressor *self,
    ...
    result = (ZstdCompressionReader *)PyObject_CallObject(
        (PyObject *)st->ZstdCompressionReaderTypeObj, NULL);
    if (!result) {
        return NULL;
    }
    ...
    zresult = ZSTD_CCtx_setPledgedSrcSize(self->cctx, sourceSize);
    if (ZSTD_isError(zresult)) {
        PyErr_Format(zstd_error(st), "error setting source source: %s",
                     ZSTD_getErrorName(zresult));
        goto except;
    }
...
static PyObject *ZstdCompressor_compress(ZstdCompressor *self, PyObject *args,
    ...
    Py_END_ALLOW_THREADS
        ...
        if (ZSTD_isError(zresult)) {
        PyErr_Format(zstd_error(st), "cannot compress: %s",
                     ZSTD_getErrorName(zresult));
        Py_CLEAR(output);
        goto finally;
    }
    else if (zresult) {
        PyErr_SetString(zstd_error(st), "unexpected partial frame flush");
        Py_CLEAR(output);
        goto finally;
    }
...
static ZstdCompressionObj *ZstdCompressor_compressobj(ZstdCompressor *self,
    ...
    result = (ZstdCompressionObj *)PyObject_CallObject(
        (PyObject *)st->ZstdCompressionObjTypeObj, NULL);
    if (!result) {
        return NULL;
    }
...
static ZstdCompressorIterator *ZstdCompressor_read_to_iter(ZstdCompressor *self,
    ...
    result = (ZstdCompressorIterator *)PyObject_CallObject(
        (PyObject *)st->ZstdCompressorIteratorTypeObj, NULL);
    if (!result) {
        return NULL;
    }
...
static ZstdCompressionWriter *ZstdCompressor_stream_writer(ZstdCompressor *self,
    ...
    result = (ZstdCompressionWriter *)PyObject_CallObject(
        (PyObject *)st->ZstdCompressionWriterTypeObj, NULL);
    if (!result) {
        return NULL;
    }
...
ZstdCompressor_chunker(ZstdCompressor *self, PyObject *args, PyObject *kwargs) {
    ...
    chunker = (ZstdCompressionChunker *)PyObject_CallObject(
        (PyObject *)st->ZstdCompressionChunkerTypeObj, NULL);
    if (!chunker) {
        return NULL;
    }
...
compress_from_datasources(ZstdCompressor *compressor, DataSources *sources,
    ...
    if (threadCount > 1) {
        ...
        if (NULL == pool) {
            PyErr_SetString(zstd_error(st),
                            "could not initialize zstd thread pool");
            goto finally;
        }
    }
    ...
    Py_END_ALLOW_THREADS
        ...
        case CompressorWorkerError_zstd:
            PyErr_Format(zstd_error(st), "error compressing item %zd: %s",
    ...
    for (i = 0; i < threadCount; i++) {
        ...
        for (j = 0; j < state->destCount; j++) {
            ...
            buffer = BufferWithSegments_FromMemory(
                st, destBuffer->dest, destBuffer->destSize, destBuffer->segments,
    ...
    result = (ZstdBufferWithSegmentsCollection *)PyObject_CallObject(
        (PyObject *)st->ZstdBufferWithSegmentsCollectionTypeObj, segmentsArg);
...
ZstdCompressor_multi_compress_to_buffer(ZstdCompressor *self, PyObject *args,
    ...
    if (PyObject_TypeCheck(data, st->ZstdBufferWithSegmentsTypeObj)) {
    ...
    else if (PyObject_TypeCheck(data, st->ZstdBufferWithSegmentsCollectionTypeObj)) {
        Py_ssize_t j;
        Py_ssize_t offset = 0;
        ZstdBufferWithSegments *buffer;
...
void compressor_module_init(PyObject *mod) {
    ...
    st->ZstdCompressorTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(mod, &ZstdCompressorSpec, NULL);
    if (!st->ZstdCompressorTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdCompressorTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "ZstdCompressor",
                          (PyObject *)st->ZstdCompressorTypeObj);
}
