...
static PyObject *DecompressionObj_decompress(ZstdDecompressionObj *self,
    ...
    if (self->finished) {
        PyErr_SetString(zstd_error(st), "cannot use a decompressobj multiple times");
        return NULL;
    }
...
void decompressobj_module_init(PyObject *module) {
    ...
    st->ZstdDecompressionObjTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            module, &ZstdDecompressionObjSpec, NULL);
    if (!st->ZstdDecompressionObjTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdDecompressionObjTypeObj) < 0) {
        return;
    }
}
