...
int decompress_input(ZstdDecompressionReader *self, ZSTD_outBuffer *output) {
    ...
    if (ZSTD_isError(zresult)) {
        ...
        PyErr_Format(zstd_error(st), "zstd decompress error: %s",
...
void decompressionreader_module_init(PyObject *mod) {
    ...
    st->ZstdDecompressionReaderTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            mod, &ZstdDecompressionReaderSpec, NULL);
    if (!st->ZstdDecompressionReaderTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdDecompressionReaderTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "ZstdDecompressionReader",
                          (PyObject *)st->ZstdDecompressionReaderTypeObj);
}
