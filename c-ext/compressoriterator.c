...
void compressoriterator_module_init(PyObject *mod) {
    ...
    st->ZstdCompressorIteratorTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            mod, &ZstdCompressorIteratorSpec, NULL);
    if (!st->ZstdCompressorIteratorTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdCompressorIteratorTypeObj) < 0) {
        return;
    }
}
