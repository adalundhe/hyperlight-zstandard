...
#define TRY_SET_PARAMETER(params, param, value)                                \
    if (set_parameter(st, params, param, value))                               \
        return -1;
...
int reset_params(ZstdCompressionParametersObject *params) {
    ZstdModuleState *st = zstd_state_from_obj((PyObject *)params);
    if (params->params) {
    ...
    return set_parameters(st, params->params, params);
}
...
#define TRY_GET_PARAMETER(params, param, value)                                \
    {                                                                          \
        ...
        if (ZSTD_isError(zresult)) {                                           \
            PyErr_Format(zstd_error(st), "unable to retrieve parameter: %s",   \
                         ZSTD_getErrorName(zresult));                          \
            return 1;                                                          \
        }                                                                      \
    }
...
CompressionParameters_from_level(PyObject *cls, PyObject *args,
                                 PyObject *kwargs) {
    ZstdModuleState *st = zstd_state_from_type((PyTypeObject *)cls);
    int managedKwargs = 0;
    int level;
    ...
    result = PyObject_New(ZstdCompressionParametersObject,
                          st->ZstdCompressionParametersTypeObj);
    if (!result) {
        goto cleanup;
    }
...
#define PARAM_GETTER(name, param)                                              \
    PyObject *ZstdCompressionParameters_get_##name(PyObject *self,             \
        ...
        if (ZSTD_isError(zresult)) {                                           \
            PyErr_Format(zstd_error(st), "unable to get compression parameter: %s", \
                         ZSTD_getErrorName(zresult));                          \
            return NULL;                                                       \
        }                                                                      \
...
void compressionparams_module_init(PyObject *mod) {
    ...
    st->ZstdCompressionParametersTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            mod, &ZstdCompressionParametersSpec, NULL);
    if (!st->ZstdCompressionParametersTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdCompressionParametersTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "ZstdCompressionParameters",
                          (PyObject *)st->ZstdCompressionParametersTypeObj);
}
