...
void constants_module_init(PyObject *mod) {
    ...
    version = PyUnicode_FromString(HYPERLIGHT_ZSTANDARD_VERSION);
    PyModule_AddObject(mod, "__version__", version);

    st->ZstdErrorType =
        PyErr_NewException("zstandard.backend_c.ZstdError", NULL, NULL);
    if (!st->ZstdErrorType) {
        return;
    }

    if (PyModule_AddObjectRef(mod, "ZstdError", st->ZstdErrorType) < 0) {
        return;
    }
