...
void compressionreader_module_init(PyObject *mod) {
    ...
    st->ZstdCompressionReaderTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            mod, &ZstdCompressionReaderSpec, NULL);
    if (!st->ZstdCompressionReaderTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdCompressionReaderTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "ZstdCompressionReader",
                          (PyObject *)st->ZstdCompressionReaderTypeObj);
}
