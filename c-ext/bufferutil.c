...
BufferWithSegments_FromMemory(ZstdModuleState *st, void *data,
    ...
    result = PyObject_New(ZstdBufferWithSegments, st->ZstdBufferWithSegmentsTypeObj);
    if (NULL == result) {
        return NULL;
    }
...
static ZstdBufferSegment *BufferWithSegments_item(ZstdBufferWithSegments *self,
    ...
    result = (ZstdBufferSegment *)PyObject_CallObject(
        (PyObject *)st->ZstdBufferSegmentTypeObj, NULL);
    if (NULL == result) {
        return NULL;
    }
...
BufferWithSegments_segments(ZstdBufferWithSegments *self) {
    ...
    ZstdBufferSegments *result = (ZstdBufferSegments *)PyObject_CallObject(
        (PyObject *)st->ZstdBufferSegmentsTypeObj, NULL);
    if (NULL == result) {
        return NULL;
    }
...
BufferWithSegmentsCollection_init(ZstdBufferWithSegmentsCollection *self,
    ...
    for (i = 0; i < size; i++) {
        ...
        if (!PyObject_TypeCheck(item, st->ZstdBufferWithSegmentsTypeObj)) {
...
void bufferutil_module_init(PyObject *mod) {
    ...
    st->ZstdBufferWithSegmentsTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            mod, &ZstdBufferWithSegmentsSpec, NULL);
    if (!st->ZstdBufferWithSegmentsTypeObj) {
        return;
    }
#if PY_VERSION_HEX < 0x03090000
    st->ZstdBufferWithSegmentsTypeObj->tp_as_buffer =
        &BufferWithSegments_as_buffer;
#endif
    if (PyType_Ready(st->ZstdBufferWithSegmentsTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "BufferWithSegments",
                          (PyObject *)st->ZstdBufferWithSegmentsTypeObj);

    st->ZstdBufferSegmentsTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            mod, &ZstdBufferSegmentsSpec, NULL);
    if (!st->ZstdBufferSegmentsTypeObj) {
        return;
    }
#if PY_VERSION_HEX < 0x03090000
    st->ZstdBufferSegmentsTypeObj->tp_as_buffer = &BufferSegments_as_buffer;
#endif
    if (PyType_Ready(st->ZstdBufferSegmentsTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "BufferSegments",
                          (PyObject *)st->ZstdBufferSegmentsTypeObj);

    st->ZstdBufferSegmentTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            mod, &ZstdBufferSegmentSpec, NULL);
    if (!st->ZstdBufferSegmentTypeObj) {
        return;
    }
#if PY_VERSION_HEX < 0x03090000
    st->ZstdBufferSegmentTypeObj->tp_as_buffer = &BufferSegment_as_buffer;
#endif
    if (PyType_Ready(st->ZstdBufferSegmentTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "BufferSegment",
                          (PyObject *)st->ZstdBufferSegmentTypeObj);

    st->ZstdBufferWithSegmentsCollectionTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            mod, &ZstdBufferWithSegmentsCollectionSpec, NULL);
    if (!st->ZstdBufferWithSegmentsCollectionTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdBufferWithSegmentsCollectionTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "BufferWithSegmentsCollection",
                          (PyObject *)st->ZstdBufferWithSegmentsCollectionTypeObj);
}
