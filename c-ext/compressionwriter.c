...
void compressionwriter_module_init(PyObject *mod) {
    ...
    st->ZstdCompressionWriterTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            mod, &ZstdCompressionWriterSpec, NULL);
    if (!st->ZstdCompressionWriterTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdCompressionWriterTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "ZstdCompressionWriter",
                          (PyObject *)st->ZstdCompressionWriterTypeObj);
}
