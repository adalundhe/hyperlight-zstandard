...
ZstdCompressionDict *train_dictionary(PyObject *self, PyObject *args,
    ...
    Py_END_ALLOW_THREADS
        ...
        PyMem_Free(dict);
        PyErr_Format(zstd_error(st), "cannot train dict: %s",
                     ZDICT_getErrorName(zresult));
        goto finally;
    }

    result = PyObject_New(ZstdCompressionDict, st->ZstdCompressionDictTypeObj);
    if (!result) {
        PyMem_Free(dict);
        goto finally;
    }
...
int ensure_ddict(ZstdCompressionDict *dict) {
    ZstdModuleState *st = zstd_state_from_obj((PyObject *)dict);
    if (dict->ddict) {
        return 0;
    }
    ...
    Py_END_ALLOW_THREADS if (!dict->ddict) {
        PyErr_SetString(zstd_error(st), "could not create decompression dict");
        return 1;
    }
...
ZstdCompressionDict_precompute_compress(ZstdCompressionDict *self,
    ...
    if (!PyArg_ParseTupleAndKeywords(
            ...
            st->ZstdCompressionParametersTypeObj, &compressionParams)) {
        return NULL;
    }
    ...
    if (self->cdict) {
        ...
        if (ZSTD_isError(zresult)) {
            PyErr_Format(zstd_error(st), "unable to free CDict: %s",
    ...
    if (!self->cdict) {
        PyErr_SetString(zstd_error(st), "unable to precompute dictionary");
        return NULL;
    }
...
void compressiondict_module_init(PyObject *mod) {
    ...
    st->ZstdCompressionDictTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(mod, &ZstdCompressionDictSpec, NULL);
    if (!st->ZstdCompressionDictTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdCompressionDictTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "ZstdCompressionDict",
