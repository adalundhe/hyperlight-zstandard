...
static PyObject *ZstdDecompressionWriter_enter(ZstdDecompressionWriter *self) {
    ...
    if (self->entered) {
        PyErr_SetString(zstd_error(zstd_state_from_obj((PyObject *)self)),
                        "cannot __enter__ multiple times");
        return NULL;
    }
...
static PyObject *ZstdDecompressionWriter_write(ZstdDecompressionWriter *self,
    ...
    while (input.pos < (size_t)source.len) {
        ...
        Py_END_ALLOW_THREADS
            ...
            PyErr_Format(zstd_error(zstd_state_from_obj((PyObject *)self)),
...
void decompressionwriter_module_init(PyObject *mod) {
    ...
    st->ZstdDecompressionWriterTypeObj =
        (PyTypeObject *)PyType_FromModuleAndSpec(
            mod, &ZstdDecompressionWriterSpec, NULL);
    if (!st->ZstdDecompressionWriterTypeObj) {
        return;
    }

    if (PyType_Ready(st->ZstdDecompressionWriterTypeObj) < 0) {
        return;
    }

    PyModule_AddObjectRef(mod, "ZstdDecompressionWriter",
                          (PyObject *)st->ZstdDecompressionWriterTypeObj);
}
